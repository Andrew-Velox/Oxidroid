...
pub fn render(f: &mut Frame, area: Rect, s: &Settings) {
    ...
    let rows: Vec<Row> = entries.iter().enumerate().map(|(i, (name, val))| {
        ...
        Row::new(vec![
            ...
            ratatui::widgets::Cell::from(*name).style(Style::default().fg(if s.focused { Color::Cyan } else { Color::DarkGray })),
    ...
    let instruction_text = if s.focused {
        vec![Span::styled("↑↓:Navigate  ", Style::default().fg(Color::DarkGray)), Span::styled("←→:Adjust  ", Style::default().fg(Color::DarkGray)), Span::styled("r:Reset  ", Style::default().fg(Color::Yellow)), Span::styled("Esc:Back", Style::default().fg(Color::Red))]
    } else {
        vec![Span::styled("Enter: Edit Settings", Style::default().fg(Color::DarkGray))]
    };
